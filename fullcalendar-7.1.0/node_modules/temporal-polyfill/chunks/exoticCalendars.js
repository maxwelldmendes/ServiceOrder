import { eraYearToYear, constrainToRange, modFloor, epochDaysToIsoDate, isoPartsToEpochDays, computeIsoInLeapYear, bindArgs, milliInUtcDay, gregoryEraOrigins, isoDateToEpochDays, computeGregoryEraFields, memoize, throwRangeError, invalidCalendar } from "./internal.js";
...
function julianDayToUmalqura(intlUmalquraData, julianDay) {
  ...
  const {year: year, month: month, day: day} = intlUmalquraData.le(epochDaysToIsoDate(julianDay - unixEpochJulianDay));
  return {
    year: year,
    month: month,
    day: day
...
const indianMeta = [ "indian", function() {
  return createArithmeticCalendar({
    ...
    J(julianDay) {
      ...
      let dayOfGregorianYear = julianDay - (isoPartsToEpochDays(gregory.year, 1, 1) + unixEpochJulianDay);
      let firstMonthDays;
    ...
    L(year, month, day) {
      ...
      let firstMonthDays;
      let julianDay;
      return computeIsoInLeapYear(gregoryYear) ? (firstMonthDays = 31, julianDay = isoPartsToEpochDays(gregoryYear, 3, 21) + unixEpochJulianDay) : (firstMonthDays = 30, 
      julianDay = isoPartsToEpochDays(gregoryYear, 3, 22) + unixEpochJulianDay), 1 === month || (julianDay += firstMonthDays + 31 * Math.min(month - 2, 5), 
